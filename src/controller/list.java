...
public class list extends HttpServlet {
	...
	for(String x:qual){
			String sql="select uid,name,post,photo from AD1 where uid=?";
		...
		if(rs.next()){
			...
			String sql2="insert into logincheck values(?,?,?,?,?,?)";
	...
	for(String x:fail){
		...
		if(ros.next()){
	     ...
	     RequestDispatcher rd=request.getRequestDispatcher("qualp2.jsp");
